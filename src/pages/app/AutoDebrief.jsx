...
export default function AutoDebrief() {
  ...
  async function loadSessions() {
    ...
    if (allSessions.length > 0) {
      ...
      if (reviewMap[latest.id]) {
        setReview(reviewMap[latest.id]);
      }
    }
    setLoading(false);
  }
  ...
  async function generateDebrief(forceNew = false) {
    ...
    // If forceNew, delete existing review first
  ...
  return (
    <PageContainer>
      {/* Header */}
      <div className="mb-6">
        <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-yellow-400/60 mb-1">AI-Powered</p>
        <h1 className="text-2xl font-black uppercase text-white">Auto Debrief</h1>
        <p className="text-xs font-mono text-slate-600 mt-1">AI analyzes your session data, timeline, and baselines to generate a post-stream review.</p>
      </div>
                  ...
                  <>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {reviews[selectedSession.id] && (
                        <span className="text-[9px] font-mono uppercase px-2 py-0.5 rounded-full bg-green-500/10 text-green-400">Reviewed ✓</span>
                      )}
                    </div>
          ...
          {selectedSession && (
            <div className="grid md:grid-cols-[1fr_300px] gap-5">
              ...
              <div className="space-y-5">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs font-mono text-slate-500 leading-relaxed mb-4">
                          The AI will analyze this session's metrics against your historical baselines, check desktop timeline data for engagement patterns, and produce a structured review.
                        </p>
                        <button onClick={() => generateDebrief(false)} disabled={generating}
                          ...
                          {generating ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Analyzing…</> : <><Brain className="w-3.5 h-3.5" /> Generate Debrief</>}
                        </button>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-mono uppercase tracking-widest text-green-400/60">
                        Reviewed {review.reviewed_at ? new Date(review.reviewed_at).toLocaleDateString() : ""}
                      </span>
                    </div>
                ...
                {/* Creator enrichment link */}
                {hasReview && !generating && (
                  <div className="flex gap-2">
                    <Link to="/app/debrief"
                      className="flex items-center gap-1.5 text-[10px] font-mono uppercase tracking-widest px-4 py-2.5 rounded-lg border border-cyan-900/20 text-slate-500 hover:text-cyan-400 hover:border-cyan-500/20 transition-all">
                      Manual Debrief →
                    </Link>
                    <a href="https://livecenter.tiktok.com/analytics/live_video" target="_blank" rel="noopener noreferrer"
                      className="flex items-center gap-1.5 text-[10px] font-mono uppercase tracking-widest px-4 py-2.5 rounded-lg border border-pink-900/20 text-pink-400/60 hover:text-pink-400 hover:border-pink-500/20 transition-all">
                      <ExternalLink className="w-3 h-3" /> TikTok Live Center
                    </a>
                  </div>
                )}
              </div>
