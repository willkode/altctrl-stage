      ...
      <div className="grid grid-cols-1 sm:flex sm:flex-wrap gap-2">
        {!stream.promo_generated && (
          <button onClick={onGeneratePromo}
            className="flex items-center justify-center gap-1.5 text-xs font-mono uppercase tracking-widest px-4 py-3.5 rounded bg-pink-500/10 border border-pink-500/30 text-pink-400 hover:bg-pink-500/20 transition-all">
            <Radio className="w-3.5 h-3.5" /> Generate Promo
          </button>
        )}
        {stream.status !== "completed" && (
          <button onClick={onLogSession}
            className="flex items-center justify-center gap-1.5 text-xs font-mono uppercase tracking-widest px-4 py-3.5 rounded bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/20 transition-all">
            Log Session
          </button>
        )}
        <Link to="/app/schedule"
          className="flex items-center justify-center gap-1.5 text-xs font-mono uppercase tracking-widest px-4 py-3.5 rounded border border-cyan-900/40 text-slate-500 hover:text-slate-300 transition-all">
          View Schedule
        </Link>
      </div>
    </div>
