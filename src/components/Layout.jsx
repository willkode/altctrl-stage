      ...
      <nav className={`fixed top-0 left-0 right-0 z-40 border-b border-cyan-900/40 bg-[#02040f]/90 backdrop-blur-sm transition-all duration-300`}>
        <div className={`max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between transition-all duration-300 ${scrolled ? 'h-[80px]' : 'h-[100px]'}`}>
          ...
          <div className="hidden md:flex items-center gap-3">
            <button onClick={() => base44.auth.redirectToLogin("/app/dashboard")} className="text-xs font-mono uppercase tracking-widest text-pink-400 hover:text-pink-300 transition-colors border border-pink-500/40 hover:border-pink-400 px-4 py-2 rounded hover:shadow-[0_0_15px_rgba(255,0,128,0.2)] transition-all flex items-center gap-1.5">
        ...
        {mobileOpen && (
          <div className="md:hidden border-t border-cyan-900/40 bg-[#02040f] px-4 py-4 space-y-3">
            ...
            <div className="flex gap-3 pt-3 border-t border-cyan-900/30">
              <button onClick={() => { setMobileOpen(false); base44.auth.redirectToLogin("/app/dashboard"); }} className="flex-1 text-xs font-mono uppercase tracking-widest text-slate-400 border border-cyan-900/40 px-4 py-2.5 rounded text-center">
                Log In
              </button>
